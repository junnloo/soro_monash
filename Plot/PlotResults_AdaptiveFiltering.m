%% Adaptive Filtering & Sensor Degradation
figure
subplot(3,1,2)
fig = plot(tv,denorm("curvature",curvature),'--',tv,denorm("curvature",xh),'-',tv,denorm("curvature",xm),'-');
set(fig,'LineWidth',1)
axis([0,tv(end),-10,160])
xlabel('Time, $t$ (s)','Interpreter','latex')
ylabel('Resultant Curvature Angle, $x$ (deg)','Interpreter','latex')
%legend('Ground Truth','GRU-AUKF','DirectGRU-MAF','Location','best');
title('Curvature Estimation')
subplot(3,1,3)
fig = plot(tv,denorm("force",force),'--',tv,denorm("force",uh),'-',tv,denorm("force",um),'-');
set(fig,'LineWidth',1)
axis([0,tv(end),-5,80])
xlabel('Time, $t$ (s)','Interpreter','latex')
...
legend('Noise Intensified ','Original','Location','best');
