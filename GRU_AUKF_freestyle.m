...
addpath('Models')
addpath('Data')
load 'Normalised No Contact Test'
...
% Fault
slp2 = -0.002*((max(flex)-min(flex))/2);
Fault2 = slp2*(tv - tv(1));
flex_fault = max(flex + Fault2, -1.3945);
...
[hxe,pxe,cxe] = ttest2(RMS_xh,RMS_xd,'Alpha',0.05,'Tail','left','Vartype','unequal');
[hue,pue,cue] = ttest2(RMS_uh,RMS_ud,'Alpha',0.05,'Tail','right','Vartype','unequal');
